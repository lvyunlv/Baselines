...
Party::Party(std::size_t p_my_id, std::size_t p_num_parties, std::size_t p_port, const std::string& dst_ip)
    ...
    for (std::size_t other_id = 0; other_id < num_parties_; ++other_id) {
        ...
        send_endpoints_.emplace_back(boost::asio::ip::make_address(dst_ip), WhichPort(my_id_, other_id));
