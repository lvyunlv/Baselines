...
public:
    ...
    PartyWithFakeOffline(std::size_t p_my_id, std::size_t p_num_parties, std::size_t p_port, const std::string& dst_ip,
                         const std::string& job_name);
...
template <IsSpdz2kShare ShrType>
PartyWithFakeOffline<ShrType>::
PartyWithFakeOffline(std::size_t p_my_id, std::size_t p_num_parties, std::size_t p_port, const std::string& dst_ip, const std::string& job_name)
    : Party(p_my_id, p_num_parties, p_port, dst_ip) {
    // Open the file for input
    std::string file_name = job_name + (job_name.empty() ? "party-" : "-party-") + std::to_string(p_my_id) + ".txt";
    input_file_.open(kFakeOfflineDir / file_name);
