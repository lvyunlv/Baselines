...
struct BertTinyModel
{
    ...
    BertTinyModel() : c_attn_w(n_layers), c_attn_b(n_layers), c_proj_w(n_layers), c_proj_b(n_layers), ffn_up_w(n_layers), ffn_up_b(n_layers), ffn_down_w(n_layers), ffn_down_b(n_layers)
    {
...
span<u64> ffn(span<u64> &x, int layer, BertTinyModel &model)
{
...
span<u64> layernorm(span<u64> &x, int layer, BertTinyModel &model)
{
    return x;
}
...
span<u64> mha(span<u64> &x, int layer, BertTinyModel &model)
{
    u64 n_token = x.size() / model.n_embd;
...
span<u64> layer(span<u64> &x, int layer, BertTinyModel &model)
{
...
span<u64> inference(span<u64> &x, BertTinyModel &model)
{
    span<u64> y = x;
...
void model_input(BertTinyModel &model)
{
    for (int i = 0; i < model.n_layers; i++)
    {
...
int main(int argc, char **argv)
{
    ...
    utils::start_timer("bert-tiny");
    auto y = inference(x, model);
    output::call(y);
    utils::stop_timer("bert-tiny");
